...
def function1():
    ...
    x = np.arange(start=1, stop=13, step=1).reshape(6, 2)
...
def function2():
    ...
    x = np.arange(start=10, stop=37, dtype='float64').reshape(3,3,3)    #wrtie your code here
...
def function3():
    ...
    filter_arr = np.logical_and((a % 5 == 0), (a % 7 == 0))
    x = a[filter_arr]
...
def function10():
    ...
    brr = np.array([1,2,3])
    subt = np.where(arr == 3, arr[0] - brr[0], (np.where(arr == 4, arr[1] - brr[1], (np.where(arr == 5, arr[2] - brr[2], arr)))))
...
def function12():
    ...
    second = []
    for x in arr:
        first = np.concatenate((first, np.repeat(x, size)))
        second = np.concatenate((second, arr))

    ans = np.concatenate((first, second))
...
def function13():
    ...
    ans = arr[ np.where( np.logical_and(arr > 5, arr < 10) ) ] 
...
def function18():
    ...
    x = np.count_nonzero(students < 101)
    return x
...
def function19():
    ...
    return output = func(X, W, b)   # Write your code here
